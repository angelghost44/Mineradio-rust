...
// ---- Settings panel ----
var _settingsModalLoaded = false;

function initSettingsPanel() {
  _settingsModalLoaded = true;
  syncSettingsUI();
}

function syncSettingsUI() {
  var qs = document.getElementById('settings-quality');
  if (qs && playbackQuality) qs.value = playbackQuality;
  _syncSettingsToggle('diy-mode-btn', 'settings-diy-btn', !!diyPlayerMode);
  _syncSettingsToggle('userCapsuleAutoHide', 'settings-capsule-btn', !!userCapsuleAutoHide);
  _syncSettingsToggle('fxFabAutoHide', 'settings-fx-btn', !!fxFabAutoHide);
  _syncSettingsToggle('playlistPanelPinned', 'settings-pl-btn', !!playlistPanelPinned);
  var sf = document.getElementById('settings-local-folder');
  if (sf) sf.value = readLocalMusicFolder() || '(未设置)';
}

function _syncSettingsToggle(varName, btnId, on) {
...
function openSettingsModal() {
  if (!_settingsModalLoaded) initSettingsPanel();
  syncSettingsUI();
...
function onSettingsChange() {
  var qs = document.getElementById('settings-quality');
  if (qs && qs.value !== playbackQuality) {
    playbackQuality = qs.value;
    try { localStorage.setItem(PLAYBACK_QUALITY_STORE_KEY, playbackQuality); } catch(e) {}
    scheduleStateSave();
    showToast('播放品质已设置为 ' + qs.options[qs.selectedIndex].text);
  }
}

function onSettingsToggleDiy() {
  toggleDiyMode();
  _syncSettingsToggle('diy-mode-btn', 'settings-diy-btn', !!diyPlayerMode);
  scheduleStateSave();
}

function onSettingsToggle(varName, btnId) {
  window[varName] = !window[varName];
  var on = !!window[varName];
  _syncSettingsToggle(varName, btnId, on);
  var keyMap = {
    userCapsuleAutoHide: USER_CAPSULE_AUTO_HIDE_STORE_KEY,
    fxFabAutoHide: FX_FAB_AUTO_HIDE_STORE_KEY,
    controlsAutoHide: CONTROLS_AUTO_HIDE_STORE_KEY,
    playlistPanelPinned: PLAYLIST_PANEL_PIN_STORE_KEY,
  };
  var key = keyMap[varName];
  if (key) { try { localStorage.setItem(key, on ? '1' : '0'); } catch(e) {} }
  if (varName === 'userCapsuleAutoHide') applyUserCapsuleAutoHideState();
  else if (varName === 'fxFabAutoHide') applyFxFabAutoHideState();
  else if (varName === 'playlistPanelPinned') applyPlaylistPanelPinnedState();
  scheduleStateSave();
}
